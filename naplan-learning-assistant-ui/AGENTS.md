...
Official-update content must come from NAP or ACARA URLs, be stored with fetch metadata and CC BY attribution, and provide summaries in all four UI languages. Full HTML page text may be mirrored for licensed general information pages. Fetch official PDFs only on demand and render their original pages inside the app; for licence-permitted general-information PDFs, pair each original page with a machine-assisted translation. ACARA-excluded materials such as demonstration tests, example questions, prompts, marking guides, SSSR guides, and example ISRs must remain original-only: never translate, adapt, mirror, or persist those PDF files locally.
...
The restored desktop shell must have rounded outer corners without adding an inset frame around the application. Use Electron's native rounded frameless window on Windows 11 and retain the Windows 10 shape-clipping fallback; true fullscreen remains square and edge-to-edge.
...
Exam text-to-speech must use a Microsoft-signed English voice installed with Windows and generate real playable audio through the desktop shell. The player timeline must come from the generated media's duration and current time, begin at 0:00 and 0%, and reset when the question changes. Never use hard-coded TTS times or decorative progress values.
